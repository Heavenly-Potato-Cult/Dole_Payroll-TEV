{{-- resources/views/office-orders/create.blade.php --}}
{{--
    Expects from OfficeOrderController@create:
        ...
        <form method="POST" action="{{ route('tev.office-orders.store') }}">
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px;">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="office_order_no" name="office_order_no"
                           value="{{ old('office_order_no') }}"
                           placeholder="e.g. OO-2025-001"
            ...
            <div class="form-group">
                ...
                <input type="text" id="destination" name="destination"
                       ...
                       placeholder="e.g. Dipolog City, Zamboanga del Norte"
                ...
                <div style="display:flex; gap:24px; margin-top:6px;">
                    @foreach (['local' => ['Local', '#E8F5E9', '#1B5E20'], 'regional' => ['Regional', '#FFF8E1', '#F57F17'], 'national' => ['National', '#E8EAF6', '#1A237E']] as $val => $meta)
                        <label style="display:flex; align-items:center; gap:8px; cursor:pointer;
                                      ...
                                      border:2px solid {{ old('travel_type', 'local') === $val ? $meta[2] : 'var(--border)' }};
                                      background:{{ old('travel_type', 'local') === $val ? $meta[1] : 'transparent' }};
                                      font-weight:600; font-size:0.88rem; color:{{ $meta[2] }};"
                               id="label_{{ $val }}">
                            <input type="radio" name="travel_type" value="{{ $val }}"
                                   {{ old('travel_type', 'local') === $val ? 'checked' : '' }}
            ...
            {{-- Remarks --}}
            <div class="form-group">
                <label for="remarks">Remarks <span class="text-muted" style="font-weight:400;">(optional)</span></label>
...
<script>
(function () {
    // Style travel type radio labels on change
    ...
    var startEl = document.getElementById('travel_date_start');
    var endEl   = document.getElementById('travel_date_end');
