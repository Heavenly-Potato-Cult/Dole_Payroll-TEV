{{-- resources/views/special-payroll/nosi-nosa-payslip.blade.php --}}
{{--
    Expects from SpecialPayrollController::nosiNosaPayslip():
      $batch     — SpecialPayrollBatch (type = nosi|nosa, status = released)
      $employee  — Employee model
      $result    — array from SalaryDifferentialService::compute()
      $typeLabel — 'Notice of Step Increment (NOSI)' | 'Notice of Salary Adjustment (NOSA)'
      $signatory — Signatory model (hrmo_designate) or null

    Adapted from differential-payslip.blade.php — nosiNosaStore()/Show()
    delegate to the same SalaryDifferentialService, so the per-month
    breakdown, mandatory-only deductions, and lack of allowance lines are
    identical to Salary Differential (see Session 2 Goal 1/2 discussion).
    The only difference is the type label, driven by $typeLabel rather
    than a hardcoded "Salary Differential" string, mirroring how
    newHirePayslip() passes $typeLabel for transferee/others.
--}}
<!DOCTYPE html>
<html lang="en">
<head>
