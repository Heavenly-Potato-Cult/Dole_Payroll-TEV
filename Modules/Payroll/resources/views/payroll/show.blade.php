    ...
    <div class="card-body" style="padding:0;">
        <div class="table-wrap">
            ...
            @php
                ...
                // - everyone else: only use the actual-attendance ratio
                //   (PayrollComputationService::computeCutoffSplit) when the
                //   MOST RECENT compute pass applied attendance — checked via
                //   last_attendance_applied (2026-08-14), a per-pass flag that
                //   gets overwritten every compute, unlike the cumulative
                //   applied_components flag the submit() gate uses. This is
                //   deliberate: unchecking "Apply Attendance" and recomputing
                //   should visibly drop the split back to an even 50/50, as if
                //   attendance were being computed for the first time, rather
                //   than staying "stuck" on whatever was last actually applied.
                ...
                foreach ($entries as $i => $entry) {
                    ...
                    } elseif ($attendanceApplied && $snapshotForEntry) {
                    ...
                    $virtualRows[] = [
                        ...
                        'split_source' => $overridePct !== null
                            ? 'override'
                            : (($attendanceApplied && $snapshotForEntry) ? 'actual' : 'pending'),
...
const actionsHtml = removeBtn ? `${payslipBtn} ${removeBtn}` : `${payslipBtn}`;
        ...
        mainRow.innerHTML = `
            ...
            <td class="text-center" style="font-size:0.74rem; white-space:nowrap; line-height:1.35;" title="${row.split_source === 'override' ? row.split_pct_1st + '% / ' + (100 - row.split_pct_1st) + '% override' : row.split_source === 'actual' ? 'Actual attendance days' : 'Even 50/50 — attendance not yet applied this batch'}">
                <div style="${row.split_source === 'override' ? 'color:var(--navy); font-weight:600;' : ''}">${formatCurrency(row.net_1st)}</div>
