...
<div class="page-header">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;">
        <a href="{{ route('allowances.index') }}" class="btn btn-outline">← Back</a>
        @if ($batch->status === 'draft')
            <a href="{{ route('allowances.batches.edit', $batch) }}" class="btn btn-outline">Edit</a>
        @endif
    </div>
</div>
...
<div class="card" style="margin-bottom:16px;">
    <div class="card-body" style="display:flex;justify-content:space-between;align-items:center;gap:16px;flex-wrap:wrap;">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
            @if ($batch->status === 'draft')
                <form method="POST" action="{{ route('allowances.batches.advance', $batch) }}">@csrf<input type="hidden" name="action" value="submit"><button class="btn btn-primary btn-sm">Submit for Review</button></form>
            @elseif ($batch->status === 'pending_review')
                <form method="POST" action="{{ route('allowances.batches.advance', $batch) }}">@csrf<input type="hidden" name="action" value="approve"><button class="btn btn-primary btn-sm">Approve</button></form>
            @elseif ($batch->status === 'approved')
                <form method="POST" action="{{ route('allowances.batches.advance', $batch) }}">@csrf<input type="hidden" name="action" value="release"><button class="btn btn-primary btn-sm">Release</button></form>
            @endif
        </div>
    </div>
</div>
